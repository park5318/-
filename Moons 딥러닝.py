...
with tf.Session() as sess:
    ...
    for step in range(5000):
        sess.run(train, feed_dict={X: X_train_moon, Y: y_train_moon, keep_prob: 1})
        if step % 1000 == 0:
            print(step, sess.run(l, feed_dict={X: X_train_moon, Y: y_train_moon, keep_prob: 1}), sess.run([W1]))

    ## test and accuracy
    h, c, a = sess.run([hypothesis, Y, accuracy], feed_dict={X: X_test_moon, Y: y_test_moon, keep_prob: 1})
    print("\nHypothesis: ", h, "\nTrue: ", c, "\nAccuracy: ", a)
